# Project_01 Roadmap / 專案進度與未來計畫

> 2026-06-01 從 memory `roadmap` 遷出至 resources/（skill 遷移）。內文舊有的 `[[memory-slug]]` 引用對應內容多已併入 `.claude/skills/project-01-workflow/` references。


**主框架完成後要做的重構 / 優化（使用者排程中，非已確認時程）：**
...
| 選項 | 範圍 | 收益 / 成本 |
|---|---|---|
| **S6 細節 iteration（進行中）** | user Pi 端測試發現的邏輯奇怪處逐個小修 | 持續優化現有非阻塞 input 行為；user-driven |
| **STT 導入** | Whisper / vosk + push-to-talk / VAD | 解放鍵盤輸入；最直接 demo 收益；前置：S6 input queue 已鋪好 ✅ |
| **HTML UI 啟動** | FastAPI + WebSocket + 前端 | 視覺收益最大；前置：S6/STT 穩定 |
| **S7 中斷邏輯**（選擇性） | 新任務終止舊任務（覆蓋 FIFO）— action / tts queue 都加 | 「快速切換」UX；race window 大、預設 FIFO 已夠用，**未必要做**（[[incremental-rebuild]] 原則 3） |
| **Cap retry redesign**（dormant） | 三輪嘗試 user 都不滿意；需更深 expectation 對齊 | 顧客錢包 conservative；待 user 重新對齊 |
| **L1-L3 dialogue 期間動作 UX 體感評估** | 純體感 + 使用者反饋 | 微調動作觸發策略 / 文案 |

**建議優先序：** S6 細節 iteration（user 已在跑）> STT > HTML UI > S7（S7 預設 FIFO 已 OK，只在「快速切換」需求出現再做）。
...
## tts.py：edge_tts ImportError 改成 noisy warning

**狀態更新（2026-05-25）：** `tts.py` 已歸檔到 `resources/examples/legacy_threading_v1/`（S1 v1→v2 重構時）。S1 v2 階段 `speak` 是 callback 注入的 print stub，無 TTS。本決議轉變為「S2 重寫 TTS 時必走 noisy 模式」。

**決議：** S2 重寫真實 TTS callback 時，import 失敗必須 **noisy**：import 階段就 `print()` 醒目警告，並考慮讓 `speak()` 首次被 call 時也再 print 一次（或乾脆 raise）。

**Why:** 2026-05-23 debug「myProgram 沒聲音」浪費了一輪完整對話才查到根因 — 真正的問題是 Python 3.7 缺 edge_tts 套件，但 silent fallback 把這個訊號吞了，使用者只看到 `[語音] xxx` 以為 speak 有 call 到。**「失敗不發聲」是這次 debug 的禍首**，未來絕對要避免。

**How to apply:**
- **時機：** S2 重建 TTS callback（incremental rebuild 第 2 步）時必走 noisy 模式
- **方向：** import 階段 print 警告 `[語音] WARN: edge_tts 未安裝，speak() 將只 print 不播音` + 考慮整段直接 `raise ImportError`（看你要 fail-fast 還是 degraded-mode）
- **延伸原則：** 整個專案禁用「silent except + 設旗標 + 後續默默 no-op」這個 anti-pattern。要嘛 raise、要嘛 noisy warn

**觸發紀錄：** 2026-05-23 myProgram TTS debug 中暴露此問題（當時 tts.py 在主路徑下用 silent fallback，現已歸檔）。
