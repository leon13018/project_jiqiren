...
| 2026-06-19 ~ 06-20 | **STT/TTS 即時化 + 連線可靠性長征**：條件式 drain + 計時診斷（`STT_TTS_TIMING`）+ prearm 首連線 + 併發 hardening×4 反思 + **zombie bug 根治（移除逐輪 Finalize，Pi 全流程驗收 ✅）** + 辨識 robustness；試後 revert 三次（warm-arecord 收不到音 / endpointing 450 不發 / 持久連線→每輪新連線**待 Pi 驗 lag 假設**） | `changelogs/changelog_2026-06-19_stt_realtime.md` |
