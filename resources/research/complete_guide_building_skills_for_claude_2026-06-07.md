# The Complete Guide to Building Skills for Claude（PDF 全文轉寫）

> **本檔性質**：`The-Complete-Guide-to-Building-Skill-for-Claude.pdf`（同目錄，Anthropic 官方指南，33 頁）的全文 markdown 轉寫。
> **轉寫方式**：本機無 PDF 工具鏈（紅線禁裝依賴），以 Python stdlib 手搓抽取器（zlib 解壓 + ToUnicode CMap 解碼 + 雙欄重排）完成；正文忠實保留英文原文。
> **誠實標註**：少數多行代碼示例在 PDF 內部結構即碎片化，抽取不完整處以「⚠️ 原文代碼示例抽取不完整」標出，未憑記憶補造。勾選框/表情符號等特殊字形已遺失。
> **轉寫日期**：2026-06-07。版權屬 Anthropic。
...
⚠️ 原文此處的完整目錄樹代碼示例抽取不完整（PDF 內部即碎片）；結構同本檔 Chapter 1「What is a skill?」四項。
...
description: Analyzes Figma design files and generates developer handoff
  documentation. Use when user uploads .fig [...] "design-to-code handoff".
```

```yaml
description: Manages Linear project workflows including sprint [...] to "create tickets".
```

⚠️ 兩例在原文均為完整 YAML 區塊，抽取有缺行（以 […] 標示）。
...
```yaml
description: Creates sophisticated multi-page documentation systems. [...] hierarchical relationships.
```
...
⚠️ 原文此處有完整 SKILL.md 範本代碼區塊，抽取不完整；可辨識的骨架為：frontmatter（`name: your-skill` 等）→ 步驟化指令 →
...
- **Be Specific and Actionable**（原文附 Good/Bad 對比代碼例，抽取不完整：Good 例含明確 data format 要求；Bad 例如「Include error handling」「Reconnect」這類含糊指令）
- **Reference bundled resources clearly**（原文例：指向 references/ 內文件並說明用途，如 rate limiting guidance、pagination patterns）
...
[⚠️ 原文 Should NOT trigger 清單抽取與排版混雜，可辨識項：]
...
3. **Create an Installation Guide**（⚠️ 原文此處的安裝步驟代碼區塊抽取碎片化，可辨識：含 Claude.ai「Upload skill」與選取 zip 資料夾步驟、Claude Code skills 目錄放置、第 4 步 Test）
...
- Good: `"The ProjectHub skill enables teams to set up complete project [...]"`（成果導向）
- Bad: `"The ProjectHub skill [...] and Markdown instructions that calls our MCP server tools."`（功能羅列）

**Highlight the MCP + skills story:**（原文例：`[...] skills teach Claude your team's sprint planning workflow.`）
...
**Example: Design-to-development handoff**（⚠️ 原文分階段代碼區塊抽取後順序殘缺，可辨識步驟：Create asset manifest／Upload all assets／Create development tasks／Attach asset links to tasks／Assign to engineering team）
...
**Example: Report generation**（可辨識步驟：Fetch data via MCP → Save to temporary file → Run validation script `scripts/check_report.py`（查 missing sections 等）→ Address each identified issue → Regenerate affected sections → Re-validate → Apply final formatting → Save final version）
...
**Example: File storage**（可辨識步驟：Check file type and size → 決策（如 temporary files 用 local storage）→ Based on decision: Call appropriate MCP tool、Apply service-specific metadata）
...
**Example: Financial compliance**（可辨識步驟：Fetch transaction details via MCP → Apply compliance rules（check sanctions lists、assess risk level）→ 依結果 call payment processing MCP tool / apply fraud checks / process transaction 或 flag for review / create compliance case → Log all compliance checks、record processing decisions）
...
- Cause: YAML formatting issue（原文列三個 `name: my-skill` 常見錯誤對照，抽取不完整——典型為缺 `---` 分隔線、縮排錯誤、引號問題）

**Error: "Invalid skill name"**

- Cause: Name has spaces or capitals
- 例：`name: My Cool Skill` ✗ → `name: my-cool-skill` ✓
...
- Add negative triggers（原文例含「…改用其他 skill instead」句式）
- Be more specific（壞例：`description: Processes documents`）
- Clarify scope（好例：`description: PayFlow payment processing for e-commerce. Use [...]`）

#### MCP 呼叫失敗（Skill loads but MCP calls fail）
...
- **Ambiguous language** — 壞例：`Make sure to validate things properly`；好例（具體可驗）：`Project name is non-empty`、`At least one team member assigned`、`Start date is not in the past`

> **Advanced technique:** For critical validations, consider bundling a script that performs the checks programmatically rather than relying on language instructions. **Code is deterministic; language interpretation isn't.** See the skills for examples of this pattern.

- **Model "laziness"** — Add explicit encouragement（例：`Take your time to do this thoroughly`）。Note: Adding this to user prompts is more effective than in SKILL.md
...
- **Reduce enabled skills** — Evaluate if you have more than 20-50 skills enabled simultaneously；Recommend selective enablement；Consider skill "packs" for related capabilities
...
**For Technical Questions:** General questions: Community forums at the Claude Developers Discord
...
description: [What it does + when to use, with] trigger phrases.
```
...
⚠️ 原文完整 YAML 區塊抽取有缺行（license / compatibility / version 等欄位在 Chapter 2 已逐欄說明）。
