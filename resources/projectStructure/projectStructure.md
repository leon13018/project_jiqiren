...
> 最後更新：2026-05-24（L2 BDD+TDD 第一輪完成：14 scenarios PASS（總 63）+ 規則補「DEGRADED-TDD-PARTIAL 容許條款」）
...
| `sales/constants.py` | S1 v2 L0+L1+L2 ✅ | L0：7 時間常數 + `PRODUCTS` + `HAWK_SLOGANS`。L1：`L1_MENU_BANNER` / `L1_HAWK_ENTER_PROMPT` / `L1_STANDBY_ENTER_PROMPT` / `SERVICE_PHONE`。L2（2026-05-24 追加）：`L2_GREETING_PROMPT` / `L2_REJECT_THANKS` / `L2_B1_CLARIFY` / `L2_B3_REASK` / `L2_B3_THIRD_REJECT` / `L2_C_ADDED` |
...
| `sales/states.py` | S1 v2 L0+L1+L2 ✅ | L0：`run_subroutine_a`。L1：`run_l1` + 4 私有。L2（2026-05-24 加）：`run_l2` 主迴圈（caller 注入 think_count + cart，return tuple `(next_state, next_think_count)`，next_state ∈ {"L1_via_subroutine_a", "L3"}）+ `_l2_exit_a` / `_l2_b3`（想一下狀態機，第 3 次跳沉默直接走 A）/ `_l2_dispatch_response`（B-3 沉默期中斷後重跑判定）。L2 dispatcher 跳過結帳意圖 → 視為無法判斷走 B-1。L3-L5 TODO |
...
| `tests/sales/test_states.py` | 2026-05-24（L0+L1+L2 TDD）| L0：4 SUB-A + FakeScheduler。L1：12 + FakeKeyboardInput + FakeOpencv。L2（2026-05-24 加）：14 個（ENTRY 1 / A 2 / B-1 1 / B-2 1 / B-3 5 / C 3 / PRIO 1）+ FakeCustomerInput inline stub；總共 30 個 |
