...
│   ├── tts.py                            # ✅ S2 同步阻塞 TTS（edge_tts 合成 + mpg123 -q 播放；fail-fast import + noisy runtime 失敗）
...
| `tts.py` | S2 ✅ | 同步阻塞 `speak()` — `asyncio.run(edge_tts.Communicate(text, voice="zh-TW-HsiaoChenNeural").save("/tmp/last_tts.mp3"))` 合成 + `subprocess.run(["mpg123","-q",path], check=True)` 阻塞播放；無 worker/queue/中斷邏輯（S4+ 才加）；`import edge_tts` fail-fast（不 silent fallback）；runtime 失敗（合成 / 播放炸）noisy print 詳細 exception type+args+階段 + return（不 raise，caller dialog 繼續下一字）；caller `main.py` speak callback 內 lazy import 避 Windows pytest 收 `test_main_decode_error.py` 時連帶觸發 ModuleNotFoundError |
