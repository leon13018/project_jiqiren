...
│       ├── test_input_reader.py          # S6 非阻塞 stdin reader 測試（8 case：basic / timeout / drain on enter / EOF sentinel / 無效 UTF-8 replace / FIFO / 繁中 / shutdown 清 queue；用 FakeByteSource 注入避開真 stdin）
│       └── test_tts_worker.py            # TtsWorker 6 case：R1 race regression / wait_idle 立即 idle / max_wait 超時 / synth fail decrement pending / Condition.notify_all / 20 thread 並發 stress（2026-05-30 加）
...
│   ├── action.py                         # ✅ S5 非阻塞動作 worker（ActionWorker daemon thread + FIFO queue + shutdown；do() 立即返回不阻塞主線程；鏡像 tts.py 結構）
│   ├── input_reader.py                   # ✅ S6 非阻塞 stdin reader（InputReader daemon thread + queue + bytes-level decode errors=replace；read(timeout) 真 timeout 解放主線程 + 根治 TextIOWrapper buffer multibyte 殘留 bug；鏡像 tts.py / action.py module-level singleton 結構）
...
└── resources/                            # 開發 / 部署參考資源（2026-05-22 重構：大部分 tracked）
    ...
    │
    ├── research/                         # tracked — 外部生態 / 工具 / 思想家調研報告（2026-05-31 加入）
    │   ├── SDD_best_practices_2026-05-31.md  # SDD (Spec-Driven Development) 多源整合：3 opus xhigh subagent 分工調研（Anthropic 官方 / 社群插件 / Karpathy 等思想家）+ 主 agent 整合 + Project_01 具體建議
    │   ├── CC-Dynamic workflows.md       # Claude Code 動態工作流程調研（之後流程優化參考用）
    │   ├── CC_large_codebases_best_practices_2026-06-01.md  # Anthropic blog「CC 在大型程式庫的最佳實踐」轉述式結構化筆記（harness 五擴充點 / 三大配置模式 / 起步清單 + 對 Project_01 啟發）
    │   └── CC-skills.md                  # Claude Code 官方 skill 文檔（frontmatter 參考 / progressive disclosure 三層 / 支援檔案 / context:fork / 動態 context 注入）— 本次 rules→skill 遷移的架構依據
    │
    ├── specs/                            # tracked — SDD 規格書集中地（flat 結構，2026-05-31 加入）
    │   ├── sdd_workflow_formalization_2026-05-31_spec.md  # SDD v2 正式化 meta-spec（首個 spec doc）
    │   ├── sdd_optimization_from_superpowers_2026-05-31_spec.md  # SDD v3 升級借鏡 superpowers meta-spec（Iron Law / 3-stage / 4-status / self-review 等 9 整合點）
    │   └── L4_v3_dual_timer_spec.md      # L4 v3 雙計時器設計（36s budget + 12s QR refresh cycle）；2026-05-31 git mv 自 plans/業務程式邏輯規劃/
    │
    ...
    │   │   ├── L4.md                     # 第 4 層：印金額 + 等掃碼（5 鏈路含客服特殊模式 / 4 次循環 / 無法判斷 fallback；v1 規格保留）
...
| `action.py` | S5 ✅ | 非阻塞動作 worker — `class ActionWorker` daemon thread + `queue.Queue` FIFO；`do(name)` 立即 `_q.put(name)` 返回（caller thread 也立即 print「[動作] xxx」保 log 時序）；worker `_loop` 從 queue 依序取 name → `Act.runAction(name)` 阻塞至動作播完 → 下一輪；`shutdown()` 程式退出時清 queue + `if Act.runningAction: Act.stopAction()` 守衛（vendor sticky 旗號污染預防，見 [[vendor-stop-action-sticky]] memory）；**預設 FIFO 不中斷**（中斷是 S7）；vendor SDK lazy import 在 `_loop` / `shutdown` 內（Pi-only，Windows pytest 不觸發 ImportError）；runtime 失敗 noisy print + continue；caller `main.py` `do_action` callback 內 lazy import 避 module-level worker singleton 過早觸發 |
...
| `tests/sales/test_input_reader.py` | 2026-05-28（S6 incremental rebuild 第 6 步）| **8 個測試**：`myProgram/input_reader.py` `InputReader` class — basic read / timeout 返 None / latest-wins drain on enter / EOF sentinel / 無效 UTF-8 換 U+FFFD / FIFO 連續讀 / 繁中 UTF-8 / shutdown 清 queue。用 `FakeByteSource` 注入式 byte source 避開真 stdin；module-level singleton `_reader = InputReader()` 仍會在 pytest collect 啟動 daemon thread（內 OSError catch swallow pytest captured stdin 警告） |
