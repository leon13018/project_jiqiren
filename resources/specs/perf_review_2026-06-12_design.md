...
- **Pi 實測 user 回報通過**（2026-06-12）：prefetch／快取即播／demo 實跑皆正常，
  兩張 pineedtodo 驗證單關閉。
- 資產 bootstrap 已完成（2026-06-12 補記）：Pi 無 GitHub 認證無法 push → 改走
  **scp 拉回 dev 端 commit**（`85d8ad6`，55 句 1.5MB，含 2 句執行期自填的動態句）；
  Pi 端 `reset --hard origin/main` 對齊、工作樹乾淨。後續文案改動的資產更新走同法：
  Pi 跑 prewarm → dev 端 scp → commit。
