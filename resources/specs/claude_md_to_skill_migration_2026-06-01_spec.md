...
.claude/skills/project-01-workflow/       ← description 常駐，body 用到才載
  ...
  references/                             ← 11 個 workflow / 領域細節，Read on demand
...
| 改 sales 對話邏輯（需領域設計背景） | `references/myprogram-sales-design.md` |
...
9. `myprogram-threading-paths.md`
10. `myprogram-sales-design.md`
11. `conventions.md`
...
| `c2_three_way_design.md` | references/myprogram-sales-design.md |
| `l4_ack_wallclock_budget_design.md` | references/myprogram-sales-design.md |
| `speak_and_wait_architecture.md` | references/myprogram-sales-design.md |
| `cancel_confirm_cross_l.md` | references/myprogram-sales-design.md |
| `service_confirm_unified.md` | references/myprogram-sales-design.md |
| `countdown_print_design.md` | references/myprogram-sales-design.md |
| `s6_non_blocking_input.md` | references/myprogram-sales-design.md |
| `confirm_default_must_be_conservative.md` | references/myprogram-sales-design.md |
| `tts_prompt_as_ux_pacing.md` | references/myprogram-sales-design.md |
| `ux_over_technical_correctness.md` | references/myprogram-sales-design.md |
| `goal_condition_design.md` | references/myprogram-sales-design.md |
...
- 命中 → 警示（非 block，列 file + 簡體字），提醒改繁體。
...
## 12. 待 user 審查重點
1. **§6.2 memory 歸屬表**——尤其 feedback 性質檔（`fix_one_path_sweep` / `confirm_default` / `ux_over_technical` / `goal_condition`）你想留 memory 還是進 skill？
2. **§4.3 references 切法**——11 個檔顆粒度 OK？`myprogram-sales-design.md` 併了 11 個 memory 會偏大（可能再拆）。
3. **skill 命名** `project-01-workflow` 可否？（repo 名 project_jiqiren，也可叫 `jiqiren-workflow`）
4. **§7.2 繁中 hook**——要不要做？（偵測簡體有誤報風險，你接受警示型即可嗎）
5. **執行分兩階段**，Phase 1 後暫停檢視 skill 內容是否需要。
