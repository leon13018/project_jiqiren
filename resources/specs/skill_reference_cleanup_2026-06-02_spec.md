...
6. **層 2 自述 + TOC**：每個 reference 開頭保留精簡 `🎯 何時讀本檔`（1-3 行；這是「兩層確認」的第 2 層——模型開檔即判要不要續讀；本身也要簡潔別變肥）。**TOC 強制規則（user 2026-06-03 定案）：任何 >30 行的 `reference/` 或 `examples/` 檔，開頭 🎯 標頭後一律加「## 目錄」（列各 `##` 章節，確保部分讀也看得到全貌）；去噪不得移除 TOC，缺的要補。** examples 保留一句「何時用」。
...
## 12. Round 3（2026-06-03）：dispatch.md 深度去噪標準推廣至全部 reference

> 觸發：user 對 `dispatch.md` 要求更深一層去噪（commit `bbdaabf`，144→95 行、−36%、EDD 零退化），驗證「比 round-2 更激進的去噪仍不傷 workflow」後，要求**用同一把尺過完其餘所有 reference**。沿用 §2 原則、§11.1 去噪三分法、§5 cluster 分批、§6 靜態檢查、§8 commit 規範、§10 EDD 機器；以下為 round-3 delta。

### 12.1 去噪標準（即 dispatch.md round-3 實證有效那把尺）

逐檔砍以下、**且只砍這些**（規則 / 表 / 範本 / Gotcha 解法 / 行為矩陣 / 計時數值 / canonical 範例 100% 逐字保留）：
- **解釋性 `Why:` 散文** → 蒸餾成一句可執行規則（拿掉模型還是會做對的就砍）。
- **跨段 / 跨檔重複** → 依 §4 概念歸屬：屬本檔留自足最小版、不屬整段砍；不靠 load-bearing 互指。
- **日期戳 / commit SHA / 版本號（v2/v3）/ episode 敘事 / 演化史** → 砍（已在 git log / `resources/changelog.md`）。
- **檔內 meta 自述 / 過度導讀 / 底部 see-also footer**（reference 間 hub 是 SKILL.md，不互指；最多一句非承載 see-also）。**TOC 不可砍**——>30 行一律保留 / 補上「## 目錄」（見 §2.6）。
- **能指回權威檔的細節**（如 Gotcha M 解法、SDD 三段迴圈機制）→ 留觸發徵兆一句 + 一句指向，完整版砍。

判準（§11.1）：一段自問「拿掉它，Claude 規劃/執行 workflow 會出錯嗎？」不會＝噪音。

### 12.2 scope（13 檔；dispatch.md 已完成不計）

- **reference/（11 檔）**：sdd.md（247）、sales-dialog-design.md（152）、myprogram-threading-paths.md（117）、sales-tts-ux.md（106）、pi-and-structure.md（102）、worktree.md（101）、incremental-rebuild.md（90）、standard-workflow.md（80）、myprogram-vendor.md（65）、bdd-tdd.md（56）、conventions.md（47）。
- **examples/（2 檔）**：spec-reviewer-prompt.md（45）、code-quality-reviewer-prompt.md（49）——範本本體 + 檢查清單 + 回報格式必留，只砍殘留贅語。
- **SKILL.md（49）**：router 已極簡，僅微調；**不重構 router、不拆多 skill**。
- 行數為現值；目標「砍完冗餘自然剩多少」，不為湊數字犧牲細節。

### 12.3 cluster 分批 + 每批 EDD（沿用 §5 / §10；baseline = iteration-2，存 iteration-4）

| cluster | 檔 | EDD 場景 |
|---|---|---|
| 1 SDD/reviewer | sdd.md + 2 examples | S1（opus + sonnet）|
| 2 process | worktree.md + standard-workflow.md + pi-and-structure.md | S2 |
| 3 threading | myprogram-vendor.md + myprogram-threading-paths.md + incremental-rebuild.md | S3 |
| 4 sales | sales-dialog-design.md + sales-tts-ux.md | S4 |
| 5 dormant/conv | bdd-tdd.md + conventions.md | S5 |
| 6 router | SKILL.md | （全 5 場景已隱含測 router；不單獨跑）|

- 每 cluster：去噪 → §6 靜態檢查（必留 grep / 連結不斷 / 無反斜線 / 無新 stale 指標）→ 跑該 cluster EDD 場景 fresh navigator + grader 判 assertion + regression_risk → 退化補回 → commit（§8）+ push（Stop hook 自動 sync）。
- **cluster 1 做完停給 user 校準去噪力度**，再用同尺續 2-6。
- 5 場景 / assertions 沿用 iteration-3 修正版（不變）；去噪移除的是非承載敘事，本不該被任何 assertion 依賴，若某 assertion 因移除 fail＝該內容 load-bearing→補回。

### 12.4 round-3 out of scope（同 §7 / §11.4）

不動 myProgram/ code、root CLAUDE.md、code_map、hooks、agents、`resources/` 其他檔；不新增 / 改名 / 刪整 reference；去噪「外移」目的地一律在 skill 目錄外（resources/）。EDD 只用 navigator + grader/comparator，不跑 package_skill。

### 12.5 round-3 完成記錄（2026-06-03）

**✅ 完成**。dispatch.md（先前 `bbdaabf` −36%）+ 5 cluster：
- 起點 `4e646c7` → cluster1 `7cf36e0`（sdd 去噪 + TOC 標準 + dispatch TOC 修回 + 2 examples TOC）→ cluster2 `9b6fc75`（worktree/standard-workflow/pi-and-structure）→ cluster3/4/5（一個 commit，vendor/threading-paths/incremental + sales-dialog/sales-tts + bdd-tdd）。

**去噪力度（user 校準：只砍真噪音）**：這些檔 round-1/2 已清過，真噪音少。實砍＝see-also footer（全檔，非-hub 互指）+ 與 root 重複的 anti-rationalization/pointer + 日期戳/SHA/episode/meta/1 段歷史壓縮。reference 本體淨去噪約 −1750 字元（最大 threading −336 / incremental −300 / sales-dialog −251 / sales-tts −238 / bdd −211 / vendor −194 / sdd ~−420）；範本/流程/矩陣/計時值/Gotcha 解法/canonical 範例/繁簡表/Iron Law/領域 know-how 逐字保留。conventions.md、SKILL.md 無噪音未動。

**TOC 標準（§2.6，user 2026-06-03 定案）**：>30 行 reference/example 一律加「## 目錄」。修回 dispatch.md（前一輪誤刪）+ 補 2 examples；SKILL.md（router 即索引）免。

**EDD（baseline iteration-2/3 → iteration-4）**：5 場景 fresh navigator（S1 另跑 sonnet）+ grader + comparator，**全 pass、零退化、`lost_load_bearing_content=[]`**。transcript 存 `resources/evals/iteration-4/`（cluster1-s1 / cluster2-s2 / cluster345-s3s4s5 result + final-consolidated）。
