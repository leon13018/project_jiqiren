...
```python
def drain_queue(q): ...                 # 共用 helper：清空 queue（tts/action/input_reader 三份相同迴圈）

class QueueWorker(ABC):
    """daemon consumer 模板：FIFO queue + thread + get→_process→on_done 迴圈。"""
    thread_name: str
    ...
    def drain(self): drain_queue(self._q)   # 共用：清 queue
```

> **W6 修正（兩處刻意收斂，本檔隨 wave 對齊）**：
> 1. **無 except-all / 無 on_error hook**：現行 TtsWorker `_loop` 只有 try/finally（未知例外殺 thread 是現狀）；ActionWorker 的 catch 住進其 `_process`。基底加 catch-all 會改 TtsWorker 行為（純重構零行為改變），不做。
...
`wait_idle` / `_proc` lock 留在 `TtsWorker` 子類別。main.py：`_S1State` + closure 集 → `TerminalSim` 類別（`callbacks()` 回 dict 餵 `logic.run(**...)`）；兩個倒數 loop 抽 `_tick_countdown(total, label, wait_tick)`（wait_tick 注入：可中斷的 `input_reader.read` 版 / `time.sleep` 版）；finally 三段 shutdown 改 importlib 迴圈。`InputReader` 不繼承（producer 形狀），只 reuse `drain_queue`。
