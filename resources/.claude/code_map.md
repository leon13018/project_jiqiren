...
- `architecture/` — 系統架構文件（**以實作現況為準**，2026-06-21 重寫）：`README.md` 索引 + `00-system-overview`（鳥瞰 / process-thread 模型 / 啟動模式）+ `10-runtime-and-workers`（main.py 編排 + 四 worker 並行）+ `20-sales-state-machine`（L0–L5）+ `30-web-mirror-and-frontend`（web 交互狀態機）；`_archive/` 收已封存的舊願景 / 契約（FastAPI 願景式，過時不參考）。`diagrams/` 報告用系統架構圖（HTML/CSS + Chromium 截圖、交付 PNG + SVG）；**製作工作流＝ `.claude/skills/architecture-diagram` skill（已改宗 `report-design-system` 淺色蠟筆風）**。佈局：`specs/`（`00-diagram-backlog` 11 張清單 + 波次 + `01–09` 各圖內容 spec，風格無關、淺色重畫可重用）、`_legacy-dark/`（**深色 legacy 歸檔**：深色 `theme/` + 深色圖 ①–⑤ 三式；保留為淺色重畫的來源素材）、各圖淺色交付 `NN-*.{png,svg}` 置主層、**共用 render 依賴 `rough.js` + `jason8.ttf`（同層平放，未來淺色 html 直接同層引用）**。狀態：**淺色 ①–⑤ 已交付**（①②③ png+svg、html 源在 report-design-system 基準；④⑤ 2026-06-23 換膚、html/png/svg 在主層、源在 `_legacy-dark/`）、⑥⑦ 已交付（html/png/svg 在主層；STT/TTS 管線）、⑨ 已交付（類別圖 State pattern；html/png/svg 在主層）、⑧ 待重畫（模組依賴；初版不滿意、html/spec 已刪除，orchestrator 自己從零重做）、⑩⑪ 待畫。
