...
- `architecture/` — 系統架構文件（**以實作現況為準**，2026-06-21 重寫）：`README.md` 索引 + `00-system-overview`（鳥瞰 / process-thread 模型 / 啟動模式）+ `10-runtime-and-workers`（main.py 編排 + 四 worker 並行）+ `20-sales-state-machine`（L0–L5）+ `30-web-mirror-and-frontend`（web 交互狀態機）；`_archive/` 收已封存的舊願景 / 契約（FastAPI 願景式，過時不參考）。`diagrams/` 報告用系統架構圖（HTML/CSS + Chromium 截圖交付 **2× PNG + SVG**，同名並存）；**製作工作流固化於 `.claude/skills/architecture-diagram`，畫圖前載入照流程走** —— **視覺風格已改宗 `report-design-system` skill 淺色蠟筆風**（舊深色霓虹 `theme/` + 已交付深色圖 ①–⑤ 為 legacy、新圖一律淺色蠟筆）：`theme/`（legacy 深色 canonical:色彩 tokens + 版面 / 卡片樣式）、`specs/`（`00-diagram-backlog` 待畫 11 張清單 + 波次 + 待辦索引；`01–05` 各圖畫圖 spec）、各圖 `NN-<topic>.{html,png,svg}`（檔名編號對應報告章節）。註:圖①②③（Wave A）+ ④⑤（Wave B）**已交付**（三式同名 `NN-*.{html,png,svg}`，2026-06-22）；⑥–⑪ 待畫（Wave C 起）。
