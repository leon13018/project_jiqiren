...
- **🔑 大圖用自訂尺寸專屬頁（使用者定）**：詳細/大張系統圖**不塞進標準 11×8.5 頁**（會縮到看不清）。改用具名 `@page`（如 `@page tallA{size:13in 17in;margin:0}` + `.page--tallA{page:tallA;width:13in;height:17in;padding:.55in}`）給該圖**一頁、依其長寬比最大化到可讀（圖寬 ~12in、約 150–180dpi）**。Chromium print-to-PDF 已實證支援同檔多種頁尺寸。各圖**先量 PNG 尺寸/aspect**（`System.Drawing` 或 pdfinfo）再定頁尺寸；tall 圖開直式大頁、wide 圖開橫式大頁。圖可超大、可跨頁，**可讀性優先**。
...
- **置位決定**：章首用內聯章頭 + §6 右上角塗鴉（非整頁 hero）以守 15–20 頁 — 已在 Architecture / Global Constraints 載明（與 spec 頁數預算一致）。
