...
- [ ] `STT_TTS_TIMING=1 python3.11 -m myProgram` 跑一輪完整點餐 → 把終端 `[計時]` 行整段**貼回來**。
  ...
  - **（2026-06-19 新增，針對「播完不能馬上說」）每輪看這兩行**：
    - `[計時] 開麥連線 Xms`：Deepgram ws 握手耗時（arm 同步阻塞這段，mic 還沒開）。
    - `[計時] 開麥→第一個音框 Y.Ys`：arecord 真正開始收音前的**裝置冷啟動死時間**（你講的開頭會掉在這段）。
    - 這兩個把空檔拆成「連線 vs 裝置」。**回報這兩個數字**（多大、哪個是大頭）→ 才決定要不要做較大的 warm-during-prompt 改造（念提示音時先暖機）。
