"""sales 常數 subpackage（2026-05-26 P8 拆分；原 constants.py 274 行已分散到 8 子模組）。

對外 import path 保持向前相容：所有 `from myProgram.sales.constants import XXX`
仍 work。subpackage 內部子模組（timing / products / keywords / l1_text / ... / l5_text）
按職責分組，新增常數時依語意放入對應子模組即可。

子模組職責：
    timing.py    — 時間常數與計數常數（秒 / 次數）
    products.py  — PRODUCTS dict + 商品追問語音模板
    keywords.py  — CONFIRM_YES/NO 關鍵字集 + HAWK_SLOGANS
    l1_text.py   — L1 文字常數（選單 / 進入提示 / SERVICE_PHONE）
