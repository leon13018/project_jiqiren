...
def run_l2(
    ...
    # 內部 unclear_count：B-1「無法判斷」累積計數（不暴露到 return shape）
    # 任何被識別的意圖（拒絕 / 想一下 / 客服 / 商品）→ reset 0
    # B-1 命中 → ++；達 UNCLEAR_MAX → speak L2_UNCLEAR_REJECT_VOICE 走鏈路 A
    unclear_count = 0
    ...
    while True:
        ...
        # 優先序 3：結帳意圖在 L2 跳過 → 視為無法判斷（B-1，++ unclear_count）
        if intent == "結帳":
            unclear_count += 1
