...
- `phonetic.py` — 拼音近音糾錯（聲韻母模糊比對 + 歧義安全閥 + 完全同音 tie-break，純函式；pypinyin 注入 seam + 缺依賴 graceful no-op。掛問數量 / 問商品出口兜底 ASR 平翹舌 / 前後鼻音混淆，並供 `product_parser` 統一 token-parser 兜底 garbled 品名與數量）。
