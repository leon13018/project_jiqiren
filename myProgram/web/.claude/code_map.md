...
- `commands.py` — `to_token(cmd: dict) -> str | None`：觸控上行結構化命令 → 對話既有消費的 token 字串（wake（開始點餐）→`t`、order→`{品名}{數量}`、checkout/confirm→keyword 集代表字、pay→`s`）；非法→None。只 import sales 常數，無 fastapi/pydantic。
