...
def _build_callbacks(state: _S1State) -> dict:
    ...
    def read_customer_input(timeout):
        ...
        2026-05-30 加：tts.wait_idle 後進 polling loop，每秒 print `timeout = N`
        （語音播完瞬間印第一個 = ceil(timeout)）。對既有 sales/ caller 行為透明
        （傾向回值 / timeout 語意完全保留）；fakeCustomerInput 測試 stub 不經此
        路徑（直接 call sales/ helper 注入），故 344 sales tests 無影響。
        """
        ...
        # 2026-05-30 加：每秒打印倒數秒數（user 要求 UX — 觀察 Pi demo 剩餘 timeout 直觀）。
        # 設計：tts.wait_idle 後即 deadline 起算 → 語音播完瞬間印第一個 ceil(timeout)。
        # 走 polling loop（input_reader.read 仍可 timeout=N 由 daemon thread 餵），
        # 每次等到下一個整秒邊界醒來 → 印新 tick → 再 read 直到下一邊界。
        # 拿到 input 立刻 break；deadline 耗盡立刻 break（回 None）。
        # timeout is None / <= 0 不適用倒數（理論上 read_customer_input caller 不會傳；
        # 但守備性 fallback 走原 single read 保證向後相容）。
        if timeout is None or timeout <= 0:
            ...
            while True:
                ...
                print(f"timeout = {ticks}")
                # 等到下一個整秒邊界（remaining=6.0 → 1.0s；remaining=5.3 → 0.3s 對齊整秒）
                wait_to_next = remaining - (ticks - 1)
                raw = input_reader.read(wait_to_next)
                if raw is not None:
                    break
        if raw is None:
