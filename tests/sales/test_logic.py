...
覆蓋：
    ...
    3. dialog 返 L1_via_subroutine_a → cart 應已空 + 觸發子例程 A + enter_hawk_immediately=True
    4. L4 非掃碼退出 → cart 應已空 + 觸發子例程 A
...
def test_logic_assert_cart_empty_violation_raises(monkeypatch):
    ...
    模擬情境：
        第一輪 run_l1 返 "L2"；
        run_dialog 在執行時把 cart 加入商品後返 "L1_via_subroutine_a"，
        run_subroutine_a stub 不清 cart；
    ...
    def stub_run_subroutine_a(**kwargs):
        # 故意不清 cart — 模擬 bug（subroutine_a 沒清 cart）
        subroutine_calls.append(True)
...
# ============================================================
# Test 3：dialog 返 L1_via_subroutine_a → cart 已空 + 子例程 A 被呼叫 + enter_hawk_immediately=True
# ============================================================

def test_logic_dialog_exits_to_subroutine_a_with_empty_cart(monkeypatch):
    """dialog 返 L1_via_subroutine_a 時：
        - cart 必須已空（dialog 自行清空）
        - run_subroutine_a 被呼叫
        - 下一輪 run_l1 收到 enter_hawk_immediately=True

    模擬情境：顧客拒絕 → dialog 清 cart 後退出到 L1_via_subroutine_a。
    """
    l1_call_args = []
    l1_call_count = {"n": 0}
    ...
    assert len(subroutine_calls) == 1, "run_subroutine_a 應被呼叫一次"
...
# ============================================================
# Test 5：L5 退出後 cart 已空 + 子例程 A 被呼叫
# ============================================================

def test_logic_l5_exit_with_empty_cart_triggers_subroutine_a(monkeypatch):
    """run_l5 完成後：
        - L5 stub 自行清 cart
        - _assert_cart_empty 不炸（cart 已空）
        - run_subroutine_a 被呼叫
    ...
    模擬情境：顧客掃碼成功 → L5 致謝 → 子例程 A → 回 L1 叫賣。
    """
    l1_call_args = []
    l1_call_count = {"n": 0}
    ...
    assert len(subroutine_calls) == 1, "L5 後 run_subroutine_a 應被呼叫一次"
...
def test_logic_enter_hawk_immediately_consumed_after_l1(monkeypatch):
    """logic.py:69 的 `enter_hawk_immediately = False` 應在 run_l1 執行後立即 reset。

    驗證三輪路徑：
        第一輪 run_l1 收到 False（初始值）
        dialog 退出 L1_via_subroutine_a → enter_hawk_immediately = True
        第二輪 run_l1 收到 True（subroutine_a 後 flag 設 True）→ 呼叫後 flag 立即 reset 為 False
        第二輪 dialog 退出 L4（不觸發 subroutine_a，不重設 flag）
        L4 完成後退出到 L1_via_subroutine_a → flag 又設 True
        第三輪 run_l1 收到 True → 終止

    核心驗證：第二輪 run_l1 收到 True 後，若 dialog 走 L4 路徑（不觸發另一次 subroutine_a），
    下一輪 flag 不會來自殘留的 True，而是由 L4 退出時重新設定。
    即：flag 每次進 L1 前都被「consume → reset → 條件性重設」嚴格管理。

    簡化版驗證：
        輪 1：run_l1 收 False，dialog 返 L1_via_subroutine_a（subroutine_a → flag=True）
        輪 2：run_l1 收 True（flag 被消費，隨即 reset=False），dialog 返 L4，
              L4 返 L1_via_subroutine_a（subroutine_a → flag=True）
    ...
    def stub_run_dialog(*, speak, print_terminal, read_customer_input, cart, think_count,
        ...
        if dialog_call_count["n"] == 1:
            # 第一輪 dialog：退出 subroutine_a → flag=True
            return ("L1_via_subroutine_a", 0)
        else:
    ...
    # 旗號來源驗證：
    #   輪 1：初始值 False（未進過 subroutine_a）
    assert l1_call_args[0] == False, "第一輪 run_l1 應收到 enter_hawk_immediately=False（初始值）"
    #   輪 2：第一輪 dialog 退出 subroutine_a 後設 True，consume-after-use reset 在本輪執行後發生
    assert l1_call_args[1] == True, "第二輪 run_l1 應收到 enter_hawk_immediately=True（第一輪 subroutine_a 後設定）"
    #   輪 3：第二輪 L4 退出 subroutine_a 後設 True — 確認旗號由 L4→subroutine_a 重新注入，非殘留
    assert l1_call_args[2] == True, "第三輪 run_l1 應收到 enter_hawk_immediately=True（第二輪 L4→subroutine_a 後設定）"
