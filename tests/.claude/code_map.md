...
- `web/` — web transport 層測試（對應 `myProgram/web/`）：`test_bus.py`（EventBus 廣播 + 斷線剔除）、`test_display.py`（display→dict 映射 / total / 未知商品不 raise）、`test_commands.py`（Phase 2 `to_token` 映射全行為：各命令→token、未知/非法→None、結帳/確認 token membership、order token 反解 parse_products）。純 stdlib，Windows 可 pytest。
