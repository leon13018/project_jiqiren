...
編寫或修改 `myProgram/` 下任何 `.py` code **必走** — 主 agent 對齊需求 → 寫 spec 到 `resources/specs/<name>_<date>_spec.md` → user approval → 派 sales-coder 帶 spec 執行 → 主 agent 審查 → 收尾。Spec 兩種規模：完整 8 段（≥3 行改動）/ Mini 5 行（≤3 行 typo / const tweak）。雙軌 TaskCreate（主 agent 高層 checklist / subagent 內部實作清單）。
...
| SDD 流程完整版（spec template 兩種規模 / 4 階段 / 雙軌 TaskCreate / sales-coder prompt 範本 / 與其他規則關係） | `.claude/rules/sdd-workflow.md` |
| SDD spec 集中存放（meta-spec 範本 + L4_v3 範本） | `resources/specs/` |
