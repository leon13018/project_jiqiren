...
- **本 skill＝風格權威 + 報告 PDF + 系統圖換膚（產線 B）；`architecture-diagram`＝從頭畫系統圖的嚴謹 production（讀實際碼 → SDD → opus → 3-QA-panel）。** architecture-diagram **單向吃**本 skill 的淺色蠟筆風（`diagram-crayon.md` / `report-pdf.md` / `render-and-qa.md` / `assets/benchmarks/`）、已無自己的主題；本 skill 維持自足、不反向依賴。分工：報告 PDF 排版 / 設計系統規範 / 換膚既有圖 → 本 skill；**從頭畫 / 重畫一張圖（要讀碼 + QA）→ architecture-diagram**。風格 / gotchas 演進改本 skill（SSOT），兩邊都受惠。
