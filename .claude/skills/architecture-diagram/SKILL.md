...
6. **質檢 QA panel（每張圖平行派 3 個 opus QA subagent）** → 一次平行派 3 個 opus QA（`model: opus`）檢同一張圖，各持互補 lens，**只讀靜態產出物（PNG + dump）、不 render、不 GetPixel**（要比的像素已在 step 5.5 的 dump `arrowSamples[]`），各自讀 spec + `render-and-qa.md §2` gotchas + **report-design-system 三張基準** + step 5.5 三檔，**先看全圖 PNG 再裁切放大 + 對 dump + 對基準**，回結構化裁決（逐項 PASS / FAIL + 缺陷座標 + 修法）：
   - **QA-A 版面 / 空間**：零覆蓋（含 legend·note·band/group 標籤，≥30px；讀 dump `overlaps[]`）、垂直置中（dump top/bot gap）、無空蕩大卡、**無大片死空白（讀 dump `fill`+`occupancy`：`bottomDeadRatio>0.12` 或 `occupancy`〔已排除 frame〕有整列/整行/2×2 全空塊 ＝ FAIL；＋對可疑格〔大 group 內部、欄頂底、貼畫布邊角〕做 ink-ratio 抽查 <~5% ＝ FAIL——binary occupancy 看不到 frame 內空洞/低 ink，別只憑目視也別只信 occupancy）**、四角無黑邊。
...
- **交付佈局**：淺色交付圖（png+svg）置 `resources/architecture/diagrams/` 主層（淺色 ①②③ 已交付；其 html 源＝ report-design-system 基準，diagrams/ 不重複放）；`specs/` 內容 spec 風格無關、可重用。**diagrams/ 已備共用 render 依賴 `rough.js` + `jason8.ttf`（同層平放）→ 未來 ⑥–⑪ 落 diagrams/ 的淺色 html 直接同層引用 `rough.js` / `jason8.ttf`、不必再複製**（骨架 `skeleton.html` 即用此平放引用）。
- **legacy（不再產出、淺色重畫的來源素材）**：`resources/architecture/diagrams/_legacy-dark/`（深色 `theme/` + 深色圖 ①–⑤ 三式；④⑤ 尚無淺色版、重畫時從這裡取 layout/座標起手）。
