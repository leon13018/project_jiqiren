...
5. **派 opus subagent 實作 → 產出物** → 把「計畫 + 主題 spec + 核對過的碼事實 + **report-design-system 風格（diagram-crayon.md + 一張基準當骨架）** + 三鐵則 + `render-and-qa.md`」交給一個 **opus** subagent（`model: opus`）：照計畫寫 `NN-*.html`（自足單檔、內聯淺色 tokens + 蠟筆濾鏡 + Rough.js；先卡片層→截圖修版面→再 SVG 箭頭層），render 並**產出 QA 用產出物**存 `resources/snapshots/`（**一律絕對路徑**）：①四角驗過的 `NN-2x.png`、②native 解析 `NN-native.png`、③bbox JSON dump `NN-bbox.json`（用 `render-and-qa.md` / `render-pipeline.md §5.5` 配方）。implementer 自己只做基本 sanity 自檢，深度自檢交 QA panel。回報產出物路徑 + 不確定處後 idle 待命。
   > 你本身已是 subagent、無法再派 sub-subagent → 自己實作 + 自檢（仍走全圖先再局部），但仍守 step 3–4 設計紀律。
6. **質檢 QA panel（每張圖平行派 3 個 opus QA subagent）** → 一次平行派 3 個 opus QA（`model: opus`）檢同一張圖，各持互補 lens，各自讀 spec + `render-and-qa.md §2` gotchas + **report-design-system 三張基準** + step 5 產出物（PNG + JSON dump），**先全圖再裁切放大 + GetPixel + 對 dump + 對基準**，回結構化裁決（逐項 PASS / FAIL + 缺陷座標 + 修法）：
   - **QA-A 版面 / 空間**：零覆蓋（含 legend·note·band/group 標籤，≥30px）、垂直置中、無空蕩大卡、無大片死空白、四角無黑邊。
   - **QA-B 箭頭 / 連線**：走線整齊流暢、線↔三角箭頭頭同色（GetPixel 線中段 vs 箭頭頭實體）、箭頭頭真觸 + 起點實接 + 無浮空線頭 / 怪鉤 / 迴圈（**每條線兩端都裁**）、線↔文字零交疊。
   - **QA-C 風格 / 內容真實性**：**對照三張基準**確認蠟筆風到位（蠟筆框抖動 + Rough hachure 填色 + 珊瑚 hero + FIG 牌 + eyebrow + 6 色語意）、文字不溢框 / 不截斷、圓角留白、字體已載入；**＋鐵則 1 內容稽核**：逐一比對節點 / 邊 / 標籤 / 數字＝ spec 核對過的事實，揪出任何捏造 / 漏 / 錯。
   **影像 token 全留在 QA subagent**；orchestrator 只聚合 3 份文字裁決。有缺陷 → 整併清單送回 step 5 implementer 修 → 重 render + 重 dump → **重派 QA panel**。loop 到 **3 個 QA 全 PASS** 才進 step 7（N 輪不收斂則 surface 使用者裁定）。
7. **給使用者驗收** → 送自檢版 PNG + 設計決策 + 三問（風格 / 內容 / 版面）。*（autonomous / 無人模式 → 自審到位即收、不阻塞。）*
8. **通過才收尾** → 匯出 **2× PNG + SVG** 進 `resources/architecture/diagrams/`、更新 `specs/`、commit（`docs(diagrams): …`）。未過回 step 5–6。
...
> - 🔴 **不可平行**：**render / 截圖**。Playwright MCP 是**單一共享瀏覽器實例** —— 兩 subagent 同時 navigate/截圖會互相把頁面導走。故 **render 階段必須序列化**：orchestrator 一次只給一張圖「獨占瀏覽器」跑完 render + 產出 3 檔，回報 + 停用後才 `GO` 下一張。（自足單檔走 `chrome --headless --screenshot file://` 直接 render、免 server，見 `render-and-qa.md §1`。）
