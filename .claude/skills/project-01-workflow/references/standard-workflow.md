...
### 1b.（條件性）更新 projectStructure.md

若本輪變更改動到專案資料結構（檔案 / 資料夾增刪移 / 改名，包括 gitignored；或修改 `.gitignore`；觸發清單見 [`pi-and-structure.md`](pi-and-structure.md) §projectStructure），主 agent 編輯 `resources/projectStructure/projectStructure.md`（目錄樹 + 職責表 + 更新紀錄），納入下一步的 `git add`。不觸發直接跳過。
