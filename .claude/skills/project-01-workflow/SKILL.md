...
> **檔案位置**：repo 檔 → **巢狀 code_map**（root `.claude/code_map.md` + 各層 `<層>/.claude/code_map.md`，逐層下沉、第一優先查）；skill 內部 → 下方路由表 + `examples/`（SDD reviewer 範本）+ `scripts/`（可執行工具：memory / code_map / CLAUDE.md 三支健檢、EDD 跨輪聚合、Pi pycache 清理）。
