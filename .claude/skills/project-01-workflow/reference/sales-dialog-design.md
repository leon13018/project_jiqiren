...
| 顧客動作 | helper | 行為 |
|---|---|---|
| **CANCEL keyword**（取消 / 取消吧 / 幫我取消…） | `DialogSession.exit_a` | 清 cart + speak `L3_REJECT_THANKS` + return `("L1_via_subroutine_a", 0)` 退出 |
| **CONTINUE keyword**（繼續 / 繼續選購…） | `DialogSession.main_loop` | **不清 cart**，重入 dialog 讓顧客繼續加單 |
...
- **keyword family**：CONTINUE/CANCEL/CONFIRM_YES + REJECT 同類一次掃（FP 防護故意不加「還要」「加買」）；sentinel `cancel_to_l1`（`_dialog_checkout_confirm` 內 cancel_confirm YES → caller `checkout_flow` 識別後直 call `DialogSession.exit_a` 退 L1，免兩輪 YES；quality_fix_w1 後識別點收斂為單一）；合成 ack voice（`L3_C2_CONTINUE_ACK` / `*_CANCEL_DECLINED_RESUME` 避免顧客失上下文）。
- **reuse 既有 helper**（零新 architecture）：`DialogSession.exit_a`（取消退出）/ `_dialog_checkout_confirm`（C-1 confirm）/ `DialogSession.main_loop`（重入）。
...
| 計時器 | 常數 | 秒 | 行為 |
|---|---|---|---|
| 總 budget | `L4_TOTAL_BUDGET` | 36 | 耗盡 → `_l4_exit_to_l1(io, cart, L4_D_FORCED_EXIT)`（speak + clear cart 退 L1） |
...
```python
budget_deadline, cycle_deadline = _l4_fresh_deadlines()                  # 36s / 12s（兩計時器同錨點）
while True:
    now = time.monotonic()
    if budget_deadline - now <= 0: return _l4_exit_to_l1(io, cart, L4_D_FORCED_EXIT)  # budget 優先
    ...
    result, pause = _l4_dispatch_response(...)
    if isinstance(result, tuple): return result
    if result == "reset":                                                # 客服 yes 繼續 → reset 兩計時器
        ...
        continue
    if pause > 0: budget_deadline += pause; cycle_deadline += pause      # cancel NO 子狀態耗時補償
...
| 拒絕 NLU | `cancel_confirm` 6s gate → YES=`_l4_exit_to_l1(.., L4_B_CANCEL_THANKS)` / NO=`CANCEL_DECLINED_NOTICE` continue | 子狀態期間**暫停**、退出後**補償** |
...
| budget 耗盡 | `_l4_exit_to_l1(.., L4_D_FORCED_EXIT)` + clear cart 退 L1 | — |
...
**4 個 explicit gate**（quality_fix_w2 統一後）：`DialogSession._dispatch` 拒絕分支（主迴圈＋沉默期兩語境單點，原 main_loop / inner_l2 / inner_l3 三點收斂）/ `_dialog_checkout_confirm`（經 `is_cancel_intent`）/ `_l4_dispatch_response` / `_dialog_unclear_final_confirmation`——reject → cancel_confirm，YES 退 L1 / NO 重 prompt continue。

**不 cover 的 2 例外**：
- **qty_followup**（`_l2_l3_qty_followup.py`）：reject 仍直接 return False fall-through 到 main loop，顧客需再喊一次才觸發 cancel_confirm。Why 不 cover：return type 是 3-tuple `(accepted, cancel_notice, control)`（2026-06-09 加 control 後），propagate 取消 sentinel 需 invasive return-type change 影響 100+ fixture（Karpathy avoid premature abstraction + user 接受 UX trade-off）。
- **C-2 CANCEL keyword**：直走 `DialogSession.exit_a` 退 L1，**不走 cancel_confirm**。Why：C-2 已是「警告 6s 倒數中」context，再加 6s confirm = 罰 12s 違反字面 promise（user 拍板 keep 快速 path）。
...
**4 個進入點**（quality_fix_w2 統一後）：`DialogSession._dispatch` 客服分支（主迴圈＋沉默期兩語境單點；YES→speak 該 mode `service_yes_prompt` + continue / NO→`DialogSession.exit_a`）｜`_l4_service_mode`（allow_scan=True；YES→return None 讓 run_l4 主迴圈 re-speak entry + reset 兩計時器 / NO→清 cart 退 L1）｜`_qty_follow_up_sub_loop`（YES→re-speak qty prompt 不計 attempts / NO→`(False, cancel_notice, None)` skip 該商品）｜`invalid_qty_reask`（YES→重 speak 無效數量 prompt + deadline 暫停補償 / NO→`"reenter_timeout"`）。對齊 user 字面：「回原鏈路 + 重印該鏈路 entry prompt（非亂答 clarify）」。
...
- **偵測點（qty==0 funnel，鏡像超量）**：
  ...
  - **`parse_products` 透出明確 0**：`parse_quantity(window, default=None)`（W1 oop_w1 合併原 `_parse_quantity_in_window`）視窗有阿拉伯數字但全為 0 → 回 `0`（非 `None`「缺數量」），對齊 `nlu.parse_quantity` B16——**否則 Pass1 `qty==0` 偵測點不可達**（「紅茶0」被當缺數量走追問，spec 前提修正 commit f1739ba）。
- **訊號往上傳**：`resolve_and_add_products` 與 `_qty_follow_up_sub_loop` return 第 3 元素 `control`；dialog caller（`DialogSession._dispatch`，兩語境單點）解 3-tuple——`exit_l1`→`DialogSession.exit_a` 退 L1；`reenter_*`→speak「prefix+當前層 entry」continue（prefix = timeout / cancel 兩種）。reenter **不**觸發 `do_action(L3)` transition（邊緣重入路徑）。
