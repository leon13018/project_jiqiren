...
**Commit 範圍**：主 agent 在 worktree 寫的 BDD spec 檔若 untracked，subagent commit 可能漏 add → prompt 內 git add 範圍明列 `tests/spec/L?_*.py`（L1 第一輪曾漏，後預防成功）。
