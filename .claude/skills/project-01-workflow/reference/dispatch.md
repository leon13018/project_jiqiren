...
```python
Agent({
  subagent_type: "sales-coder",  # frontmatter: model opus（無 effort → 繼承 session effort）, skills: [karpathy, TDD, project-01-workflow]
  description: "...",
  prompt: "...任務描述 + 業務規格 + 既有 helper reuse 點 + git add 範圍 + commit message 範本..."
})
```

不必再傳 `model: "opus"`（frontmatter 已設）、不必再 paste karpathy SKILL 內容（已預載）；effort 依官方機制繼承當前 session，毋需 prompt 內塞 xhigh（要更高 effort 才手動要求）。
...
不必再塞 `model: "opus"`、不必再 paste karpathy SKILL 內容 — frontmatter 已預設；effort 依官方機制繼承當前 session，毋需 prompt 塞 xhigh。
