...
> **語音 echo 顯示 `SALES_VOICE`**（2026-06-21，反轉自原 `SALES_QUIET`）：**預設隱藏**終端正常機器人 echo（`[語音]`/`[動作]`/`[模擬提示]`）；`SALES_VOICE=1` 才顯示。**導航**（`print_terminal`：選單 / L4 結帳明細 / SERVICE_PHONE / prompts）與 echo **一併歸此旗標 gate**（2026-06-21 Option A：原「保留導航」反轉——demo 走 web/語音、終端導航也是雜訊；**鍵盤選單操作需 `SALES_VOICE=1`**）；只有錯誤 `⚠️`（tts/action `_print_failure`）+ 啟動/退出行（`[模式]`/`[webui]`/`[系統]`，純 `print`）恆顯示、不受旗標影響。main/tts/action 各自讀 `SALES_VOICE`（沿用 STT_TTS_TIMING precedent，不新增跨模組 import），`if _VOICE:` 只包 print——**gate 外**的 `_worker.say/do`（語音/動作 dispatch）、控制流不受影響（隱藏下機器人仍說/動）。預設 0 = 隱藏（demo 走 web UI/觸控，終端 echo 與鏡像重複是雜訊）。countdown 與本旗標獨立。

> **輸入控制 `--hawk` flag + `SALES_KEYBOARD` gate + 啟動防呆**（2026-06-21）：demo 預設不用鍵盤——模式入口用 CLI flag（`python3.11 -m myProgram --web --hawk` 跳主選單直接進叫賣；未來新模式加新 flag），`start_hawk` 經 `main._run_wiring → logic.run → SalesMachine.enter_hawk_immediately` 串接（複用既有「交易後連續叫賣」機制，只是把首次進場初值從 hardcode False 改由 flag 帶入）。鍵盤由 `SALES_KEYBOARD`（env，預設 0=關）gate `input_reader` 的 stdin reader thread；關時只 web/語音經 `inject()` 驅動（`inject`/`read`/`shutdown` 不受 gate 影響），退出用 Ctrl+C。**啟動防呆**：無 mode flag 且鍵盤關（會卡在無法操作的選單）→ `_run_wiring` 印訊息 early return；合法組合 = 有 `--hawk`，或無 flag 但 `SALES_KEYBOARD=1`（選單 + 鍵盤）。
