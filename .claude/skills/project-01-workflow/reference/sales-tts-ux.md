...
> **語音 echo 顯示 `SALES_VOICE`**（2026-06-21，反轉自原 `SALES_QUIET`）：**預設隱藏**終端正常機器人 echo（`[語音]`/`[動作]`/`[模擬提示]`）；`SALES_VOICE=1` 才顯示。保留導航（`print_terminal` 螢幕文字 / 選單 / prompts）+ 錯誤 `⚠️`（恆顯示、不受旗標影響）。main/tts/action 各自讀 `SALES_VOICE`（沿用 STT_TTS_TIMING precedent，不新增跨模組 import），`if _VOICE:` 只包 print——**gate 外**的 `_worker.say/do`（語音/動作 dispatch）、控制流不受影響（隱藏下機器人仍說/動）。預設 0 = 隱藏（demo 走 web UI/觸控，終端 echo 與鏡像重複是雜訊）。countdown 與本旗標獨立。
