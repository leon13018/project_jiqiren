...
> **安靜模式 `SALES_QUIET`**（2026-06-20）：`SALES_QUIET=1` 藏終端正常機器人 echo（`[語音]`/`[動作]`/`[opencv]`/`[模擬]`/`[模擬提示]`），保留導航（`print_terminal` 螢幕文字 / 選單 / `進入叫賣模式`）+ 錯誤 `⚠️` + 開場小抄。main/tts/action 各自讀 `SALES_QUIET`（沿用 STT_TTS_TIMING precedent，不新增跨模組 import），`if not _QUIET:` 只包 print——**gate 外**的 `_worker.say/do`（語音/動作 dispatch）、`opencv_*` 狀態寫入、控制流不受影響（quiet 下機器人仍說/動/偵測）。預設 0 = 全顯示。countdown 與本旗標獨立。
