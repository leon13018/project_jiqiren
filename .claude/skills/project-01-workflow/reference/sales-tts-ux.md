...
**TtsWorker（`myProgram/tts.py`）race-free 原語**：`threading.Condition` + `_pending: int` counter（取代 v1 `_active: bool` polling，消除 R1 race）。`say()` 原子 `with cv: _pending += 1; _q.put(text)`；`_loop()` try/finally 包 `_process_text`，finally `_pending -= 1`，==0 則 `cv.notify_all()`。
...
> **演化教訓**：v1（`8e3aa67`，主 agent 直接 patch）踩 3 bug——P0 unbounded wait（synth 卡網路永久阻塞毀 L4 budget）/ P1 R1 race（q.get 返回與 `_active=True` 之間誤判 idle）/ M2 budget 被 speak 時間吃掉。→ v2（`c418004`，派 sales-coder）修 P0（max_wait）+ P1（Condition+_pending）+ budget caller 用 `speak_and_wait` 顯式控 deadline。→ v3（`075309a`）read/sleep callback 重新引入 wait_idle（P0/P1/M2 已解才安全）。**教訓：worker/wire-up 結構改動派 sales-coder、別主 agent 直接 patch**（見 [sdd.md](sdd.md) / [dispatch.md](dispatch.md) worker-level）。
...
> 註：L4 entry budget 本架構落地當時為 60s，後簡化為 30s、code 再演進到 **v3 36s 雙計時器**（見 [sales-dialog-design.md](sales-dialog-design.md) L4 budget + `resources/specs/L4_v3_dual_timer_spec.md`）。speak_and_wait 機制本身不受該值變動影響。
...
**Why**：2026-05-27 主 agent 曾把 `L3_C1_CHECKOUT_GO="好的，為您結帳"` 列為冗餘推薦移除，user 拒絕並解釋：大廠在需計算時間的程式跑過場效果（loading 進度條）減緩用戶焦慮，看似冗餘但比卡頓提升體驗。**技術冗餘 ≠ UX 冗餘**（ack 提早填補 silence = 顧客知道機器人在處理 = 焦慮↓）。
...
**Why**：2026-05-27 user 教育主 agent——「認清技術極限，運算時間避免不了 → 用心理學掩蓋不足；客戶不在乎背後怎麼實現，只要當下體驗好就是好。」第一輪把 ack speak 當冗餘**不是視角錯，是順序錯**（應先 UX 後技術）。
