...
> **🎯 何時讀本檔**：要判斷本輪是否需 Pi 端操作 → 寫 pineedtodo；結構變動要更新 code_map；或查部署資訊 / Pi 環境陷阱。
...
- **結構變動 → 更新「變動所在那一層」的 `.claude/code_map.md`**（巢狀 code_map：每層各有索引、只列直接子項目；新增加一行、刪除撤行、改名改路徑）。
  - **巢狀判準（可優雅失敗）**：改哪層就更新哪層的 code_map。**若該層沒有自己的 `.claude/code_map.md`**（只下沉到部分層）→ 通常**不需動**：其有 code_map 的最近祖先是粗顆粒、不列個別檔（例：新增 `tests/sales/` 測試檔，`tests/` code_map 只記「`sales/` — 回歸網」不列檔，故不動）。**只有變動影響到某層 code_map 已寫出的條目**（新增 / 刪除 / 改名該層的**直接**子項目）才更新該層。
