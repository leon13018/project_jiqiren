...
- **結構文檔健檢（兩支，皆只報告不改檔、exit 0/1/2；gitignored 檔不進 worktree——從主 checkout 跑或 `-RepoRoot` 指過去）**：
  - `scripts/codemap-health.ps1`——各層 code_map 路徑引用驗存活（解析：本層→同行目錄→祖先層）。死引用 = 該層 code_map 漏更新——修 code_map 而非刪引用。
  - `scripts/claudemd-health.ps1`——CLAUDE.md 行數預算（root ≤100 / 子層 ≤60，90% 預警）+ 死引用（本層→root→葉名遞迴）；語意維度（分層分配 / 零重複）script 不判，agent 對照 root 維護原則人工掃。
  - 觸發：結構變動收尾順手跑、或使用者喊「code_map / CLAUDE.md 健檢」。
