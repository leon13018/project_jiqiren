...
> **使用者偏好（2026-05-23 實證）**：多線程重構後「按 y 行為不一致 / 語音動作有時被打斷 / 切換 idle-customer 會亂」，連兩輪 patch 仍有 bug → 使用者判「重做反而省力」，事實證明正確（race window 太多補不完）。
...
## 核心原則（每步遵守）
1. **每步只一變數**（一輪只加一 thread/queue/IO/中斷）→ bug 99% 跟新加層有關，範圍立即收斂。
...
**Why（2026-05-23）**：曾用 `cmd_queue` + `customer_queue` + `has_customer` 在 producer 端分流（`if has_customer: customer_queue.put() else cmd_queue.put()`）。「按 y 行為不一致」根因：使用者打字時間點剛好落在 `customer_mode` timeout 退出後幾 ms，`has_customer` 已設回 False 但使用者以為仍在 customer mode → 字進錯 queue → 下次切換時殘留字被當新輸入。
...
**Why（2026-05-23）**：`hawking_loop` 每輪無條件 `Act.stopAction()` 清舊任務 → 每次 vendor 動作（wave_hand/bow/point_screen）一進去就被旗號打斷，只有頭部舵機動、身體沒動。讀 `ActionGroupControl.py:118` 才發現 sticky 機制。
...
- 歷史：commit `a95507f`（守衛修復點）；`resources/examples/legacy_threading_v1/README.md`（若存在）。廠商禁改與 API 見 [myprogram-vendor.md](myprogram-vendor.md)。

---

**相關 reference**：[bdd-tdd.md](bdd-tdd.md)（S1 業務邏輯開發）/ [myprogram-vendor.md](myprogram-vendor.md) / [myprogram-threading-paths.md](myprogram-threading-paths.md) / [sdd.md](sdd.md)
