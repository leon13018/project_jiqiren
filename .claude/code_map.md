...
- `.claude/` — Claude Code 配置：`code_map.md`(本檔)、`skills/`（`project-01-workflow` 等）、`hooks/`（自動化 hook + `NOTES.md`）、`workflows/`（dynamic workflow 腳本：`skill-edd-regression` EDD 回歸 harness，場景庫在 `resources/evals/`）、`agents/`（`sales-coder`）、`settings*.json`。
